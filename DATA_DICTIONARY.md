...
This file is a run-specific copy of the parameter input file used to generate a simulation. It is automatically copied into the output directory at the start of each run to improve provenance and reproducibility.

### Purpose
- Preserves the exact parameter values used for that simulation run
- Allows users to verify run conditions after completion
- Supports FAIR and reproducible computational practices

### Notes
- The contents should match the input parameter file supplied at run start
- This file is stored inside the simulation output directory
- It serves as a provenance record for that individual run
