"""
Plot_Fig_4_publication.py
─────────────────────────
Generates publication-quality Figure 4 (aggregate count with vs. without
oligomer clearance) from the two ensemble CSV files.

Outputs (written to the same directory as this script):
  Figure_4.eps   – vector graphic (journal preferred format)
  Figure_4.tif   – 600 dpi RGB TIFF, LZW-compressed (combination artwork)
  Figure_4.pdf   – vector PDF (transparency-safe backup)
...
DATA_DIR   = SCRIPT_DIR.parent / "data" / "Figure_4"
OUT_DIR    = SCRIPT_DIR

DATA_FILES = {
    "NoClearance": DATA_DIR / "No_Oligomer_Clearance" /
                   "Appending_Aggregate_Count_No_Oligomer_Clearance.csv",
    "Clearance":   DATA_DIR / "Oligomer_Clearance" /
                   "Appending_Aggregate_Count_Oligomer_Clearance.csv",
}
for label, path in DATA_FILES.items():
    if not path.exists():
        sys.exit(f"ERROR: data file not found: {path}\n"
                 f"       Check that data/Figure_4/ exists in the repository root.")
...
# ── Colorblind-safe palette (Wong 2011) ───────────────────────────────────────
COLORS = {
    "NoClearance": "#0072B2",   # blue
    "Clearance":   "#D55E00",   # vermilion
}
LINE_STYLES = {
    "NoClearance": "-",
    "Clearance":   "--",
...
LABELS = {
    "NoClearance": "No oligomer clearance",
    "Clearance":   "Oligomer clearance (p\u202f=\u202f0.05)",
}
...
t, m_nc, s_nc, n = load(DATA_FILES["NoClearance"])
_, m_cl, s_cl, _ = load(DATA_FILES["Clearance"])
...
series = [
    ("NoClearance", t, m_nc, s_nc),
    ("Clearance",   t, m_cl, s_cl),
]
for key, ts, mean, sem in series:
    ax.plot(ts, mean,
            color=COLORS[key], linestyle=LINE_STYLES[key],
            linewidth=1.6, label=LABELS[key], zorder=3)
...
ax.set_ylabel(f"Mean aggregate count (\u00b1\u202fSEM, n\u202f=\u202f{n})", labelpad=4)

leg = ax.legend(loc="lower right", borderpad=0.6, handlelength=2.0,
